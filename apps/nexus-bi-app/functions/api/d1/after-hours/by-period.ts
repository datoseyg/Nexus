...
// GET /api/d1/after-hours/by-period - "Horas fuera de horario por mes",
// equivalente D1 de /api/dashboard/after-hours/by-period. Ver
// docs/CLOUDFLARE_D1_MIGRATION.md.
//
// Período = primeros 7 caracteres de start_time_local ("YYYY-MM") en vez de
// strftime() de SQLite - start_time_local se exportó como el toString() de
// un TIMESTAMP de DuckDB (puede traer sufijo de offset tipo "-04"), que
// strftime() de SQLite no siempre parsea bien; los primeros 7 caracteres
// son siempre el año-mes sin ambigüedad.
export const onRequestGet: PagesFunction<Env> = async context => {
  try {
    ...
    const result = await context.env.DB
      .prepare(
        `SELECT
           CASE WHEN start_time_local IS NULL OR start_time_local = '' THEN '(sin fecha)' ELSE substr(start_time_local, 1, 7) END AS key,
