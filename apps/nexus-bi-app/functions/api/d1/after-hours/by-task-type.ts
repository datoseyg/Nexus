...
// GET /api/d1/after-hours/by-task-type - "Horas fuera de horario por tipo
// de tarea", equivalente D1 de /api/dashboard/after-hours/by-task-type. Ver
// docs/CLOUDFLARE_D1_MIGRATION.md.
export const onRequestGet: PagesFunction<Env> = async context => {
  try {
    const url = new URL(context.request.url);
    const filters = parseAfterHoursFilters(url);
    const { where, params } = buildConditions(filters, ["task_type IS NOT NULL AND task_type != ''"]);
