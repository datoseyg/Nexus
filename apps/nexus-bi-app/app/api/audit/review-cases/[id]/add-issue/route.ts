...
// Gate B - Familia 4: agrega un issue a un caso ya existente
// (governance.fn_add_issue_to_review_case, sql/097). Un issue solo puede
// tener una membresía activa a la vez (B53) - la función rechaza con
// VALIDATION_ERROR si ya pertenece a otro caso.
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    const rows = await runGovernanceQuery<{ fn_add_issue_to_review_case: Record<string, unknown> }>(
      "app_corrections",
      "SELECT governance.fn_add_issue_to_review_case($1::uuid, $2, $3, $4, $5, $6, $7::uuid, $8::integer)",
      [user.id, user.role, reviewCaseId, issueId, reason, idempotencyKey, correlationId, expectedVersion ?? null]
    );

    const result = rows[0]?.fn_add_issue_to_review_case;
    ...
    if (mapped) {
      await recordCommandAttempt({
        correlationId,
        commandType: "review-case:add-issue",
        actorUserId: user.id,
