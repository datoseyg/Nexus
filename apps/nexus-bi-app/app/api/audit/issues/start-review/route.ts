...
// Gate B - ciclo de vida de incidencias (Familia 3): iniciar revisión.
// Capacidad audit:review. Transición OPEN -> IN_REVIEW, sin verificación
// asociada (no es una corrección, es una marca de "esto se está evaluando").
...
export async function POST(request: NextRequest) {
  ...
  let body: StartReviewRequestBody;
  try {
  ...
  try {
    const rows = await runGovernanceQuery<{ fn_start_review: Record<string, unknown> }>(
      "app_corrections",
      "SELECT governance.fn_start_review($1::uuid, $2, $3, $4, $5, $6::uuid, $7::integer)",
      [user.id, user.role, issueId, typeof reason === "string" ? reason : null, idempotencyKey, correlationId, expectedVersion ?? null]
    );

    const result = rows[0]?.fn_start_review;
    ...
    if (mapped) {
      await recordCommandAttempt({
        correlationId,
        commandType: "issue:start-review",
        actorUserId: user.id,
